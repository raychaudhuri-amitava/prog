...
int main(int argc,char**argv)
{
    ...
    FILE *done=NULL; //done upto now file.
    ...
    while(!feof(cnfg))
    {
        ...
        if(cdflg==0)
        {
            cmd=(char*)malloc((12+urlcnt)*sizeof(char));
            cmd=strcpy(cmd,"youtube-dl ");
            cmd=strcat(cmd,url);
            system(cmd);
            system("clear");
        }
